...
describe("local-first reference deletion contract", () => {
  it("does not ask for confirmation and retries marker cleanup without a receipt", async () => {
    ...
    expect(cleanup).toContain("if (receipt === undefined) { await this.cleanupUnusedMarkers(); return; }");
